...
import { credentials, type Diploma } from "@/data/credentials";

export function CredentialsSection() {
  const [selected, setSelected] = useState<Diploma | null>(null);

  const titulo = credentials.find((d) => d.category === "titulo");
  const especializaciones = credentials.filter((d) => d.category === "especializacion");
  const cursos = credentials.filter((d) => d.category === "curso");
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-16">
        ...
        {titulo && (
          <div className="max-w-sm mx-auto w-full">
            <DiplomaCard diploma={titulo} variant="featured" onClick={setSelected} />
          </div>
        )}
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {especializaciones.map((d) => (
                <DiplomaCard key={d.id} diploma={d} variant="medium" onClick={setSelected} />
              ))}
            </div>
          </div>
        )}
        ...
        {cursos.length > 0 && (
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {cursos.map((d) => (
                <DiplomaCard key={d.id} diploma={d} variant="compact" onClick={setSelected} />
              ))}
            </div>
          </div>
        )}
